# phac-nml/submitdatairidanext: Usage
...
sample,fastq_1,fastq_2,biosample_accession
SAMPLE1,sample1_R1.fastq.gz,sample1_R2.fastq.gz,SAMN12345678
SAMPLE2,sample2_R1.fastq.gz,sample2_R2.fastq.gz,SAMN12345679
SAMPLE3,sample3_R1.fastq.gz,,SAMN12345680
```
...
```bash
nextflow run phac-nml/submitdatairidanext -profile docker -params-file params.yaml
```
...
First, go to the [phac-nml/submitdatairidanext page](https://github.com/phac-nml/submitdatairidanext) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
